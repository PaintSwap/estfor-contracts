...
import {EquipPosition, ItemInput, LotteryWinnerInfo, ExtendedBoostInfo, StandardBoostInfo} from "../contracts/globals/players.sol";
import {GuaranteedReward, RandomReward} from "../contracts/globals/rewards.sol";
import {BRONZE_AXE, LUCKY_POTION, LUCK_OF_THE_DRAW, PRAY_TO_THE_BEARDIE, PRAY_TO_THE_BEARDIE_2, PRAY_TO_THE_BEARDIE_3, CLAN_BOOSTER, CLAN_BOOSTER_2, CLAN_BOOSTER_3} from "../contracts/globals/items.sol";
...
contract WishingWellTest is FullGameStack {
  ...
  function testClaimingPreviousClaimsWorksUpToTwoOtherLotteriesAgo() public {
    ...
    for (uint256 i; i < 2; ++i) {
      _nextDayAndFulfill();
      uint256 interimPlayerId = _createPlayer(address(this), 1, string.concat("my name ser", vm.toString(i)), false);
  ...
  function _addBoostItems() private {
    ...
    for (uint256 i; i < items.length; ++i) {
      ...
      items[i].equipPosition = i < 2
        ...
        : i < 5
          ? EquipPosition.GLOBAL_BOOST_VIAL
          : EquipPosition.CLAN_BOOST_VIAL;
      items[i].boostType = i == 3 || i == 6
        ? BoostType.COMBAT_XP
        : i == 4 || i == 7
  ...
  function _basicWoodcuttingActions() private returns (QueuedActionInput[] memory queuedActions) {
    ...
    queuedActions[0] = QueuedActionInput(
