...
contract CosmeticsTest is FullGameStack {
    ...
    function _addWoodcuttingAvatar(bool includeEvolved) private {
        ...
        avatars[0] = AvatarInfo(
            "Woodcutting champ", "", includeEvolved ? "10.jpg" : "10010.jpg", [Skill.WOODCUTTING, Skill.NONE]
        );
        if (includeEvolved) {
            avatarIds[1] = 10_010;
    ...
    function _setupBasicWoodcutting() private returns (QueuedActionInput memory queuedAction) {
        ...
        actions[0] = ActionInput({
            ...
            info: ActionInfo(
                uint8(Skill.WOODCUTTING), false, 3_600, 0, 0, BRONZE_AXE, WOODCUTTING_MAX, 100, 0, false, true, 0
            ),
        ...
        queuedAction = QueuedActionInput(
            Attire(0, 0, 0, 0, 0, 0, 0, 0), WOODCUTTING_ACTION, 0, 0, BRONZE_AXE, 0, ACTION_TIMESPAN, 0, 0
        );
    }
    ...
    function _hasLog(
        ...
        for (uint256 i; i < logs.length; ++i) {
            if (logs[i].emitter != emitter || logs[i].topics.length != expectedTopics || logs[i].topics[0] != topic0) continue;
    ...
    function _cosmeticInfos(uint16 tokenId, EquipPosition position, uint24 avatarId)
        private
        pure
        returns (CosmeticInfo[] memory infos)
    {
