...
contract LockedBankVaultsTest is FullGameStack {
  ...
  bytes32 internal constant BATTLE_RESULT_TOPIC =
    keccak256(
  ...
  function testAttackBackLoseAndThenWin() public {
    ...
    _assignLockedVaultCombatants(
      CLAN_ID,
      _uint64s(playerId, ownerPlayerId, erinPlayerId, frankPlayerId),
      playerId,
      ALICE
    );
  ...
  function _decodeBattleResult(
    Vm.Log memory log
  ) private view returns (LockedBankVaultsBattleResultData memory result) {
    return battleResultDecoder.decode(log.data);
  }

  function _fulfillBattleResults(
    uint256 requestId,
    uint256 seed,
    bool seeded
  ) private returns (LockedBankVaultsBattleResultData[] memory results) {
    vm.recordLogs();
    if (seeded) {
  ...
  function _uint32s(
    ...
    uint256 e,
    uint256 f
  ) private pure returns (uint32[] memory values) {
    values = new uint32[](6);
    values[0] = uint32(a);
    values[1] = uint32(b);
    values[2] = uint32(c);
