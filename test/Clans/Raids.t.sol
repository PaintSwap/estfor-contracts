...
import {ILockedBankVaultsBattleResultDecoder, RaidBattleOutcomeData} from "../../contracts/test/interfaces/LockedBankVaultsBattleResultDecoder.sol";
...
contract RaidsTest is FullGameStack {
  ...
  bytes32 private constant OUTCOME_TOPIC =
    keccak256("RaidBattleOutcome(uint256,uint256,uint256,uint256,uint256,uint16[],uint256,bool,uint256[],uint256[])");
  ...
  function _fulfillOutcome(
    uint256 requestId,
    uint256 seed,
    bool seeded
  ) private returns (RaidBattleOutcomeData memory out) {
    vm.recordLogs();
