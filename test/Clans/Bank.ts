...
describe("Bank", function () {
  ...
  it("Withdraw (Distribute) to many users", async function () {
    ...
    await clans.connect(alice).createClan(playerId, clanName, discord, telegram, 2, 2);
    ...
    for (let i = 0; i < tokenIds.length; ++i) {
      ...
      let exists = false;
      for (let j = 0; j < nftsInfos.length; ++j) {
        const nftInfo: any = nftsInfos[j];
        if (to == nftInfo.to) {
          // Already exists
          exists = true;
      ...
      if (!exists) {
        nftsInfos.push({tokenIds: [tokenId], amounts: [amount], to: to});
      }
    }

    const bank = await Bank.attach(clanBankAddress);

    await expect(bank.connect(alice).withdrawItemsBulk(nftsInfos, playerId)).to.emit(bank, "WithdrawItemsBulk");
