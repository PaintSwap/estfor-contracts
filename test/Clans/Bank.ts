...
describe("Bank", function () {
  ...
  it("Should be able to deposit items up to max capacity and only treasurers withdraw", async function () {
    ...
    const balanceBefore = await itemNFT.balanceOf(alice, EstforConstants.BRONZE_SHIELD);
    await expect(bank.connect(alice).withdrawItems(alice, playerId, [EstforConstants.BRONZE_SHIELD], [1]))
    ...
    await expect(
      bank.connect(owner).withdrawItems(alice, newPlayerId, [EstforConstants.BRONZE_SHIELD], [1])
  ...
  it("Should be able to withdraw non-transferable boosts", async function () {
    ...
    const bank = (await Bank.attach(clanBankAddress)) as unknown as Bank;
    await expect(bank.connect(alice).withdrawItems(alice, playerId, [EstforConstants.SKILL_BOOST], [1]))
      .to.emit(bank, "WithdrawItems")
  ...
  it("Should be able to deposit and withdraw tokens", async function () {
    ...
    await expect(bank.connect(alice).depositToken(alice, playerId + 1n, brush, 1000)).to.be.revertedWithCustomError(
      bank,
      "NotOwnerOfPlayer"
    );

    await expect(bank.connect(alice).depositToken(alice, playerId, brush, 1000))
    ...
    await expect(bank.connect(alice).withdrawToken(playerId, bob, bobPlayerId, brush, 500))
    ...
    await expect(bank.connect(alice).withdrawToken(playerId, bob, bobPlayerId, brush, 750))
      .to.emit(bank, "WithdrawToken")
    ...
    await expect(
      bank.connect(alice).withdrawToken(playerId, bob, bobPlayerId, brush, 250)
    ).to.be.revertedWithCustomError(bank, "NotClanAdmin");
  });
  ...
  it("Should be able to deposit and withdraw other ERC1155 NFTs", async function () {
    ...
    const bobPlayerId = await createPlayer(playerNFT, avatarId, bob, "bob", true);
    await expect(bank.connect(alice).withdrawNFT(playerId, bob, bobPlayerId, erc1155, tokenId, 400))
    ...
    await expect(
      bank.connect(alice).withdrawNFT(playerId, bob, bobPlayerId, erc721, tokenId, 1)
    ).to.be.revertedWithCustomError(bank, "NFTTypeNotSupported");

    await expect(
      bank.connect(alice).withdrawNFT(playerId, alice, bobPlayerId, erc721, tokenId, 1)
  ...
  describe("Withdraw tokens to many users", function () {
    it("Withdrawer is not owner of player", async function () {
      ...
      await expect(
        bank.connect(alice).withdrawTokenToMany(playerId + 1n, [alice], [playerId], brush, [250])
    ...
    it("Must be at least treasurer to withdraw", async function () {
      ...
      await expect(
        bank.connect(alice).withdrawTokenToMany(playerId, [alice], [playerId], brush, [250])
      ).to.be.revertedWithCustomError(bank, "NotClanAdmin");
    });
    ...
    it("Length mismatch", async function () {
      ...
      await expect(
        bank.connect(alice).withdrawTokenToMany(playerId, [alice], [], brush, [250])
      ).to.be.revertedWithCustomError(bank, "LengthMismatch");

      await expect(
        bank.connect(alice).withdrawTokenToMany(playerId, [alice], [playerId], brush, [])
      ).to.be.revertedWithCustomError(bank, "LengthMismatch");
    });
    ...
    it("Owner mismatch with the player id", async function () {
      ...
      await expect(
        bank.connect(alice).withdrawTokenToMany(playerId, [alice], [playerId + 1n], brush, [250])
      ).to.be.revertedWithCustomError(bank, "ToIsNotOwnerOfPlayer");
    });
