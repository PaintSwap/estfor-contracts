...
describe("Instant VRF actions", function () {
  ...
  it("Any inputs should be burnt (do multiple)", async function () {
    ...
    expect(await itemNFT.balanceOfs(alice.address, [BRONZE_ARROW, IRON_ARROW, ADAMANTINE_ARROW])).to.deep.eq([2, 1, 0]);
  });
  ...
  it("Do multiple instant actions at once", async function () {
    ...
    expect(await itemNFT.balanceOfs(alice.address, [BRONZE_ARROW, IRON_ARROW, ADAMANTINE_ARROW])).to.deep.eq([4, 2, 0]);
  ...
  it("Not paying the request cost", async function () {
    ...
    const actionAmount = 1;
    await expect(
      instantVRFActions.connect(alice).doInstantVRFActions(playerId, [instantVRFActionInput.actionId], [actionAmount], {
  ...
  describe("Edit", function () {
    it("Must be owner to edit an action", async function () {
      ...
      await instantVRFActions.addActions([instantVRFActionInput]);
      await expect(instantVRFActions.connect(alice).editActions([instantVRFActionInput])).to.be.revertedWithCustomError(
        instantVRFActions,
        "CallerIsNotOwner"
      );
  ...
  it("Check random rewards (many)", async function () {
    ...
    for (let i = 0; i < instantVRFActionInput.randomRewards.length - 1; ++i) {
      const chance = instantVRFActionInput.randomRewards[i].chance - instantVRFActionInput.randomRewards[i + 1].chance;
