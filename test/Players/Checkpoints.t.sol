...
import {ActionChoiceInput, ActivePlayerInfo, CheckpointEquipments, EquipPosition, FullAttireBonusInput, ItemInput, PendingQueuedActionState} from "../../contracts/globals/players.sol";
import {GuaranteedReward, RandomReward} from "../../contracts/globals/rewards.sol";
import {NONE, BRONZE_AXE, BRONZE_PICKAXE, MAGIC_FIRE_STARTER, FIRE_MAX, NET_STICK} from "../../contracts/globals/items.sol";
...
contract CheckpointsTest is FullGameStack {
  ...
  function testRemovingFullAttireInvalidatesBonusEvenWhenItemIsReminted() public {
    ...
    _addAttireBonus(Skill.WOODCUTTING, NATURE_MASK, NATURE_BODY, NATURE_BRACERS, NATURE_TROUSERS, NATURE_BOOTS, 3, 0);
    action.attire = Attire(NATURE_MASK, NONE, NATURE_BODY, NATURE_BRACERS, NATURE_TROUSERS, NATURE_BOOTS, NONE, NONE);
    itemNFT.mintBatch(
      ALICE,
  ...
  function _setupGatheringActions()
    private
    returns (QueuedActionInput memory fishing, QueuedActionInput memory woodcutting, QueuedActionInput memory mining)
  {
  ...
  function _setupGatheringAction(
    uint16 actionId,
    Skill skill,
    uint16 handItem,
    uint16 handMax,
