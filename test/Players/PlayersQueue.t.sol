...
import {ActionInput, ActionInfo, ActionQueueStrategy, QueuedAction, QueuedActionInput, GUAR_MUL, RATE_MUL} from "../../contracts/globals/actions.sol";
import {ActionChoiceInput, EquipPosition, ItemInput, PendingQueuedActionState, Player} from "../../contracts/globals/players.sol";
...
contract PlayersQueueTest is FullGameStack {
  ...
  function _setupGathering(
    ...
    uint16 reward,
    uint32 minXP
  ) private returns (QueuedActionInput memory q) {
  ...
  function _actionInfo(
    ...
    uint16 minHand,
    uint16 maxHand
  ) private pure returns (ActionInfo memory i) {
    i.skill = uint8(skill);
    i.xpPerHour = xp;
  ...
  function _itemInputs(
    uint16 id,
    EquipPosition p,
    Skill s,
    uint32 xp,
    uint16 health
  ) private pure returns (ItemInput[] memory v) {
    v = new ItemInput[](1);
    v[0].tokenId = id;
    v[0].equipPosition = p;
    v[0].skill = s;
  ...
  ) private pure returns (QueuedActionInput[] memory v) {
