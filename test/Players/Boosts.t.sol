...
import {ActionChoiceInput, BoostInfo, EquipPosition, ExtendedBoostInfo, ItemInput, PendingQueuedActionState, StandardBoostInfo} from "../../contracts/globals/players.sol";
import {GuaranteedReward, RandomReward, PendingRandomReward} from "../../contracts/globals/rewards.sol";
import {NONE, XP_BOOST, GATHERING_BOOST, COMBAT_BOOST, LUCK_OF_THE_DRAW, PRAY_TO_THE_BEARDIE, PRAY_TO_THE_BEARDIE_2, PRAY_TO_THE_BEARDIE_3, CLAN_BOOSTER, CLAN_BOOSTER_2, CLAN_BOOSTER_3, BOOST_STABLILIZER_10, SHADOW_SCROLL, BRONZE_AXE, BRONZE_SWORD, NET_STICK, BRONZE_PICKAXE} from "../../contracts/globals/items.sol";
...
contract BoostsTest is FullGameStack {
  ...
  function testAddBoostPartialConsume() public {
    ...
    assertEq(
      ...
      queuedAction.timespan + (queuedAction.timespan * 10) / 100
    );
    // Check the drops are as expected
    assertEq(itemNFT.balanceOf(ALICE, LOG), (queuedAction.timespan * rate) / (3600 * GUAR_MUL));
  }
  ...
  function testGlobalXPBoost() public {
    ...
    emit PlayersBase.ConsumeGlobalBoostVial(
      ALICE,
      playerId,
      BoostInfo(uint40(vm.getBlockTimestamp()), 120, 50, PRAY_TO_THE_BEARDIE, BoostType.ANY_XP)
    );
    vm.prank(ALICE);
  ...
  function testClanXPBoost() public {
    ...
    emit PlayersBase.ConsumeClanBoostVial(
      ...
      bobPlayerId,
      clanId,
      BoostInfo(uint40(vm.getBlockTimestamp()), 120, 50, CLAN_BOOSTER, BoostType.ANY_XP)
    );
    vm.prank(BOB);
  ...
  function testGlobalBoostOverrideWithOverlappingBoosts() public {
    ...
    emit PlayersBase.ConsumeGlobalBoostVial(
      ALICE,
      0,
      BoostInfo(uint40(vm.getBlockTimestamp()), secondBoostDuration, 30, PRAY_TO_THE_BEARDIE_2, BoostType.COMBAT_XP)
    );
    vm.prank(ALICE);
