...
import {ActionChoiceInput, EquipPosition, FullAttireBonusInput, ItemInput, PendingQueuedActionState} from "../../contracts/globals/players.sol";
import {GuaranteedReward, RandomReward} from "../../contracts/globals/rewards.sol";
import {NONE, BRONZE_AXE, BRONZE_PICKAXE, MAGIC_FIRE_STARTER, FIRE_MAX, SKILL_BOOST, SHADOW_SCROLL} from "../../contracts/globals/items.sol";
...
contract NonCombatTest is FullGameStack {
  ...
  function testFiremaking() public {
    ...
    assertEq(players.getPlayerXP(playerId, Skill.FIREMAKING), queuedAction.timespan / (rate / (mintAmount * RATE_MUL)));
  ...
  function testStealMany() public {
    ...
    uint256 xpPerHour = 2;
    uint256 numHours = 4;
    QueuedActionInput memory queuedAction = _setupThieving(uint24(xpPerHour), 100, rewards, uint24(numHours * 1 hours));
  ...
  function testStealSuccessPercentMany() public {
    ...
    uint256 xpPerHour = 2;
    uint256 numHours = 2;
    QueuedActionInput memory queuedAction = _setupThieving(
      uint24(xpPerHour),
      successPercent,
      rewards,
      uint24(numHours * 1 hours)
    );
  ...
  function testThievingFullNatuowEquipment() public {
    ...
    uint256 numHours = 2;
    QueuedActionInput memory queuedAction = _setupThieving(uint24(xpPerHour), 0, rewards, uint24(numHours * 1 hours));
    queuedAction.attire = Attire(
      NATUOW_HOOD,
      NONE,
      NATUOW_BODY,
    ...
    _addFullAttireBonus(Skill.THIEVING, NATUOW_HOOD, NATUOW_BODY, NATUOW_BRACERS, NATUOW_TASSETS, NATUOW_BOOTS, 3, 100);
  ...
  function testCraftingMoreThanOneRate() public {
    ...
    _assertSinglePending(
      state,
      _equipment2(ROPE, 1, SAPPHIRE, 20),
      _equipment1(SAPPHIRE_AMULET, 1),
      rate / RATE_MUL,
      11
    );
    _process();
  ...
  function testCraftingMultipleOutputAmount() public {
    ...
    _assertSinglePending(
      state,
      _equipment2(ROPE, 2, SAPPHIRE, 40),
      _equipment1(SAPPHIRE_AMULET, outputAmount * 2),
      7200,
      7200
    );
    _process();
  ...
  function testAlchemyFinishOneItem() public {
    ...
    itemNFT.mintBatch(
      ALICE,
      _uints3(SHADOW_SCROLL, NATURE_SCROLL, PAPER),
  ...
  function _setupCooking(
    uint8 successPercent,
    uint256 minLevel
  ) private returns (QueuedActionInput memory queuedAction, uint256 rate) {
    rate = 100 * RATE_MUL;
    ActionInput memory action;
    action.actionId = 1;
  ...
  function _setupThieving(
    uint24 xpPerHour,
    uint8 successPercent,
    RandomReward[] memory rewards,
    uint24 timespan
  ...
  function _setupArrowMaking(Skill skill) private returns (QueuedActionInput memory queuedAction, uint256 rate) {
    ...
    queuedAction = _setupRecipe(
      skill,
      rate,
      _uint16s(BRONZE_ARROW_HEAD, ARROW_SHAFT, FEATHER),
      _uint24s(1, 1, 2),
      BRONZE_ARROW,
      1,
      3600
  ...
  function _queuedAction(
    ...
    uint16 choiceId,
    uint24 timespan
  ) private pure returns (QueuedActionInput memory queuedAction) {
  ...
  ) private pure returns (Equipment[] memory equipment) {
  ...
  function _actions(
    QueuedActionInput memory first,
    QueuedActionInput memory second
  ) private pure returns (QueuedActionInput[] memory actions) {
  ...
  function _actions(
    QueuedActionInput memory first,
    QueuedActionInput memory second,
    QueuedActionInput memory third
  ) private pure returns (QueuedActionInput[] memory actions) {
