...
import {ActionInput, ActionInfo, ActionQueueStrategy, QueuedActionInput, GUAR_MUL, SPAWN_MUL} from "../../contracts/globals/actions.sol";
import {ActionChoiceInput, EquipPosition, ItemInput, PendingQueuedActionState, MAX_UNIQUE_TICKETS} from "../../contracts/globals/players.sol";
import {GuaranteedReward, RandomReward, PendingRandomReward, XPThresholdReward} from "../../contracts/globals/rewards.sol";
...
contract RewardsTest is FullGameStack {
  ...
  function testRandomBytesTicketLimits() public {
    ...
    assertEq(randomnessBeacon.getRandomBytes(MAX_UNIQUE_TICKETS, timestamp, timestamp - 1 days, playerId).length, 128);
    vm.expectRevert();
  ...
  function testMixedThievingCombatThievingProcessedBeforeOracle() public {
    (
      QueuedActionInput memory combat,
      QueuedActionInput memory thieving,
      uint24 numSpawned
  ...
  function testTicketExcessRareRewardsUseChanceMultiplier() public {
    ...
    uint256 scaledChance = ((uint256(numSpawned) / SPAWN_MUL) * 23 * chance * 1e6) /
      MAX_UNIQUE_TICKETS /
      type(uint16).max;
  ...
  function _setupWoodcutting(
    uint256 rate,
    uint24 xpPerHour,
    uint16 rewardTokenId,
    uint24 timespan
  ) private returns (QueuedActionInput memory action) {
  ...
  function _thresholds(
    ...
    uint16 tokenId,
    uint24 amount
  ) private pure returns (XPThresholdReward[] memory values) {
  ...
  function _queuedAction(
    ...
    uint24 timespan,
    CombatStyle style
  ) private pure returns (QueuedActionInput memory action) {
