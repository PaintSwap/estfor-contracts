...
import {ActionChoiceInput, AvatarInfo, EquipPosition, ItemInput, PendingQueuedActionState} from "../../contracts/globals/players.sol";
...
abstract contract CombatTestBase is FullGameStack {
  ...
  function _setupMagic() internal returns (QueuedActionInput memory queuedAction, uint256 rate, uint256 numSpawned) {
    ...
    _addCombatAction(
      COMBAT_ACTION,
      uint24(numSpawned),
      CombatStats(3, 0, 0, 5, 0, 0, 0),
      BRONZE_ARROW,
      uint16(rate),
      false
    );

    ActionChoiceInput memory choice = _inputChoice(
      Skill.MAGIC,
      uint24(RATE_MUL),
      SHADOW_SCROLL,
      1,
    ...
    returns (QueuedActionInput memory queuedAction, uint256 rangedPlayerId, uint256 rate, uint256 numSpawned)
  {
    ...
    items[5] = _food(COOKED_MINNUS, 12);
    itemNFT.addItems(items);
    itemNFT.mintBatch(ALICE, _uints(BRONZE_SHIELD, COOKED_MINNUS, BRONZE_ARROW, IRON_ARROW), _uints(1, 1000, 200, 100));
  ...
  function _actionInfo(
    ...
    uint24 xpPerHour,
    bool choiceRequired
  ) internal pure returns (ActionInfo memory info) {
  ...
  function _combatQueue(
    uint16 actionId,
    uint16 choiceId,
    uint16 food,
    uint16 hand
  ) internal pure returns (QueuedActionInput memory action) {
    action.actionId = actionId;
