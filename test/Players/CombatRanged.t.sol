...
import {ActionQueueStrategy, QueuedActionInput, GUAR_MUL, RATE_MUL, SPAWN_MUL} from "../../contracts/globals/actions.sol";
...
contract CombatRangedTest is CombatTestBase {
  ...
  function testRangedAttack() public {
    ...
    assertEq(
      itemNFT.balanceOf(ALICE, NATURE_SCROLL),
  ...
  function testDeadKillAllButInsufficientFood() public {
    (QueuedActionInput memory action, uint256 food) = _deadSetup(
      CombatStats(3, 0, 0, 1, 0, 0, 0),
      100,
      100,
      3 hours,
      true
  ...
  function _deadSetup(
    ...
    uint24 timespan,
    bool helmet
  ) private returns (QueuedActionInput memory action, uint256) {
    _addCombatAction(
      COMBAT_ACTION,
