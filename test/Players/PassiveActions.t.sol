...
contract PassiveActionsTest is FullGameStack {
  ...
  function testStartingNewActionWhenPreviousFinishedOracleCalled() public {
    ...
    vm.expectEmit(false, false, false, true, address(passiveActions));
    emit IPassiveActions.ClaimPassiveAction(playerId, ALICE, 2, _uints(MAGICAL_LOG, BRONZE_ARROW), _uints(10, 2), true);
    vm.expectEmit(false, false, false, true, address(passiveActions));
    emit IPassiveActions.StartPassiveAction(playerId, ALICE, 2, 3, 0, vm.getBlockTimestamp());
    _start(2, 0);
    assertEq(itemNFT.balanceOf(ALICE, MAGICAL_LOG), 10);
  ...
  function _randomInput(
    uint16 id,
    uint8 days_,
    uint8 skip,
    uint16 guaranteedId,
    uint16 rate
  ) private pure returns (IPassiveActions.PassiveActionInput memory input) {
    input = _input();
    input.actionId = id;
  ...
  function _inputs(
    IPassiveActions.PassiveActionInput memory a
  ) private pure returns (IPassiveActions.PassiveActionInput[] memory x) {
  ...
  function _inputs(
    IPassiveActions.PassiveActionInput memory a,
    IPassiveActions.PassiveActionInput memory b
  ) private pure returns (IPassiveActions.PassiveActionInput[] memory x) {
  ...
  function _guaranteed3(
    ...
    uint16 c,
    uint16 cr
  ) private pure returns (GuaranteedReward[] memory x) {
