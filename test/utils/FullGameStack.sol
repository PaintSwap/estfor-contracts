...
import {IPlayersImplProcessActions as PlayersImplProcessActions} from "../../contracts/interfaces/IPlayersImplProcessActions.sol";
...
abstract contract FullGameStack is EstforTest {
  ...
  function _deployCoreInfrastructure() internal {
    ...
    Bridge bridgeImplementation = Bridge(_deployArtifact("contracts/Bridge/Bridge.sol:Bridge", abi.encode(lzEndpoint)));
    bridge = Bridge(
    ...
    treasury = Treasury(
      _deployUUPS(address(treasuryImplementation), abi.encodeCall(Treasury.initialize, (IBrushToken(address(brush)))))
    );
  ...
  function _deployNFTsAndSocialSystems() internal {
    ...
    WishingWell wishingWellImplementation = WishingWell(_deployArtifact("contracts/WishingWell.sol:WishingWell"));
    wishingWell = WishingWell(
      _deployUUPS(
        address(wishingWellImplementation),
        bytes.concat(
          WishingWell.initialize.selector,
          abi.encode(address(brush), address(playerNFT), address(treasury), address(randomnessBeacon), address(clans)),
  ...
  function _deployPetSystems() internal {
    ...
    petNFT = PetNFT(
      _deployUUPS(
        ...
        bytes.concat(
          ...
          abi.encode(address(treasury), address(randomnessBeacon), uint40(1), address(bridge), address(adminAccess)),
          abi.encode(true),
          _dynamicTail(abi.encode("ipfs://"))
        )
      )
    ...
    PetNFTReroll petNFTRerollImplementation = PetNFTReroll(_deployArtifact("contracts/PetNFTReroll.sol:PetNFTReroll"));
  ...
  function _deployPlayers() internal {
    ...
    PlayersImplRewards rewardsImplementation = PlayersImplRewards(
      _deployArtifact("contracts/Players/PlayersImplRewards.sol:PlayersImplRewards")
    );
    playersImplRewards = address(rewardsImplementation);
    PlayersImplMisc miscImplementation = PlayersImplMisc(
      _deployArtifact("contracts/Players/PlayersImplMisc.sol:PlayersImplMisc")
    );
    playersImplMisc = address(miscImplementation);
    PlayersImplMisc1 misc1Implementation = PlayersImplMisc1(
      _deployArtifact("contracts/Players/PlayersImplMisc1.sol:PlayersImplMisc1")
    );
  ...
  function _deployPromotionAndActionSystems() internal {
    ...
    InstantActions instantActionsImplementation = InstantActions(
      _deployArtifact("contracts/InstantActions.sol:InstantActions")
    );
    ...
    InstantVRFActions instantVRFActionsImplementation = InstantVRFActions(
      _deployArtifact("contracts/InstantVRFActions.sol:InstantVRFActions")
    );
  ...
  function _deployPVPAndRaids() internal {
    ...
    PVPBattleground pvpBattlegroundImplementation = PVPBattleground(
      _deployArtifact("contracts/PVPBattleground.sol:PVPBattleground")
    );
      ...
      payable(
        _deployUUPS(
          ...
          bytes.concat(
            ...
            abi.encode(address(players), address(itemNFT), address(clans), address(mockVRF), 8 hours, address(brush)),
  ...
  function _deployCombatAndClanSystems() internal {
    LockedBankVaults lockedBankVaultsImplementation = LockedBankVaults(
      _deployArtifact("contracts/Clans/LockedBankVaults.sol:LockedBankVaults")
    );
    ...
    Territories territoriesImplementation = Territories(_deployArtifact("contracts/Clans/Territories.sol:Territories"));
    territories = Territories(_deployUUPS(address(territoriesImplementation), _territoriesInitializer()));

    CombatantsHelper combatantsHelperImplementation = CombatantsHelper(
    ...
    PassiveActions passiveActionsImplementation = PassiveActions(
      _deployArtifact("contracts/PassiveActions.sol:PassiveActions:via-ir")
    );
  ...
  function _deployBanksAndAuxiliarySystems() internal {
    ...
    bankRegistry = BankRegistry(
      _deployUUPS(address(bankRegistryImplementation), abi.encodeCall(BankRegistry.initialize, ()))
    );
    ...
    cosmetics = Cosmetics(
      _deployUUPS(
        ...
        abi.encodeCall(Cosmetics.initialize, (address(this), IItemNFT(address(itemNFT)), PlayerNFT(address(playerNFT))))
      )
    );
    ...
    globalEvents = GlobalEvents(
      _deployUUPS(
        ...
        abi.encodeCall(GlobalEvents.initialize, (address(this), Players(address(players)), IItemNFT(address(itemNFT))))
      )
    );
  }
  ...
  function _wireItemAndClanSystems() internal {
    ...
    lockedBankVaults.initializeAddresses(
      Territories(address(territories)),
  ...
  function _createPlayer(
    ...
    bool makeActive
  ) internal returns (uint256 createdPlayerId) {
  ...
  function _createPlayer(
    ...
    vm.recordLogs();
    vm.prank(account);
    (bool success, bytes memory returndata) = address(playerNFT).call(
      _playerMintCalldata(avatarId, heroName, upgrade, makeActive)
    );
    if (!success) {
