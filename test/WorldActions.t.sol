...
contract WorldActionsTest is EstforTest {
  ...
  function _choice(
    ...
    uint16 inputTokenId,
    uint24 inputAmount
  ) private pure returns (ActionChoiceInput memory choice) {
  ...
  function _guaranteedRewards(
    uint16 itemTokenId,
    uint16 rate
  ) private pure returns (GuaranteedReward[] memory rewards) {
    rewards = new GuaranteedReward[](1);
    rewards[0] = GuaranteedReward({itemTokenId: itemTokenId, rate: rate});
  }

  function _randomRewards(
    uint16 itemTokenId,
    uint16 chance,
    uint8 amount
  ) private pure returns (RandomReward[] memory rewards) {
