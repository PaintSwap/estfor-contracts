...
import {BRONZE_SWORD, BRONZE_AXE, MAGIC_FIRE_STARTER, NET_STICK, BRONZE_PICKAXE, TOTEM_STAFF, BASIC_BOW} from "../contracts/globals/items.sol";
...
contract PlayerNFTTest is FullGameStack {
  ...
  function _decodeBase64(bytes memory data) private pure returns (bytes memory result) {
    ...
    for (uint256 i; i < data.length; i += 4) {
      uint256 value = (_base64Value(data[i]) << 18) |
