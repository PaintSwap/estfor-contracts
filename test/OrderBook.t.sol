...
contract OrderBookTest is EstforTest {
  ...
  function _orders(
    ...
    for (uint256 i; i < length; ++i) {
      orders[i] = IOrderBook.LimitOrder({side: side, tokenId: ORDERBOOK_TOKEN_ID, price: price, quantity: quantity});
    }
  }
  ...
  function testAddsBuyAndSellAndReturnsPriceLevels() public {
    ...
    IOrderBook.Order[] memory asks = orderBook.allOrdersAtPrice(
  ...
  function testMarketBuyCostMatchingBalancesAndExhaustion() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        IOrderBook.FailedToTakeFromBook.selector,
        ALICE,
  ...
  function testCancelAndMakeLimitOrdersBothSides() public {
    ...
    orderBook.cancelAndMakeLimitOrders(
      _uints(1),
      _cancel(IOrderBook.OrderSide.Buy, PRICE),
      _order(IOrderBook.OrderSide.Buy, PRICE + 1, 12)
    );
    assertEq(orderBook.allOrdersAtPrice(IOrderBook.OrderSide.Buy, ORDERBOOK_TOKEN_ID, PRICE).length, 0);
    IOrderBook.Order[] memory orders = orderBook.allOrdersAtPrice(
      IOrderBook.OrderSide.Buy,
      ORDERBOOK_TOKEN_ID,
      PRICE + 1
    );
  ...
  function testConsumedAndReaddedPriceLevelPreservesTombstoneOffset() public {
    ...
    IOrderBook.Order[] memory orders = orderBook.allOrdersAtPrice(IOrderBook.OrderSide.Buy, ORDERBOOK_TOKEN_ID, PRICE);
    assertEq(orders.length, 1);
  ...
  function testERC20CallbackCannotReenterWhileFundingBuyOrder() public {
    ...
    reentrantOrderBook.setTokenIdInfos(_uints(ORDERBOOK_TOKEN_ID), _tokenInfo(ORDERBOOK_TICK, ORDERBOOK_MIN_QUANTITY));
