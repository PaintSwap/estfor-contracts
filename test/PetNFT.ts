...
describe("PetNFT", function () {
  ...
  it("external_url when not in beta", async function () {
    const {adminAccess, brush, treasury, dev, royaltyReceiver, alice, randomnessBeacon, PetNFT, startPetId, bridge} =
  ...
  it("name & symbol", async function () {
    const {petNFT, adminAccess, brush, dev, royaltyReceiver, treasury, randomnessBeacon, PetNFT, startPetId, bridge} =
      await loadFixture(deployContracts);
  ...
  describe("Editing", function () {
    ...
    it("Cannot use a reserved pet name", async function () {
      const {petNFT, playerId, brush, editNameBrushPrice, alice} = await loadFixture(deployContracts);
      ...
      const isProbablyReserved = await petNFT.isPetNameReserved(reservedName);
      expect(isProbablyReserved).to.be.true;

      const isNotReserved = await petNFT.isPetNameReserved(unreservedName);
      expect(isNotReserved).to.be.false;

      // should get custom error HeroNameIsReserved(name) if name is reserved
      await expect(petNFT.connect(alice).editPet(playerId, petId, reservedName)).to.be.revertedWithCustomError(
        petNFT,
        "PetNameIsReserved"
      );

      const isSecondNameReserved1 = await petNFT.isPetNameReserved(secondReservedName);
      ...
      for (let i = 0; i < FALSE_POSITIVE_CHECKS && i < reservedNames.length; i++) {
        ...
        const shouldNotBeReserved = await petNFT.isPetNameReserved(notReservedName);
