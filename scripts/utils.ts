import {BaseContract, Contract, ContractTransactionReceipt, ContractTransactionResponse, Network} from "ethers"
...
const getEventLog = async (
  ...
  return receipt.logs
    ...
    .find((parsedLog) => parsedLog?.name === eventName)?.args
}
...
export const calculateClanBankAddress = async (
  clanId: number,
  bankFactoryAddress: string,
  bankBeaconAddress: string
): Promise<string> => {
  const salt = ethers.zeroPadValue(ethers.toBeHex(clanId), 32)
  const initializeCalldata = ethers.id("initialize()").slice(0, 10)
  const encodedArgs = ethers.AbiCoder.defaultAbiCoder().encode(
    ["address", "bytes"],
    [bankBeaconAddress, initializeCalldata]
  )
  const beaconProxy = await ethers.getContractFactory("BeaconProxy")
  const initCodeHash = ethers.keccak256(beaconProxy.bytecode + encodedArgs.slice(2))
