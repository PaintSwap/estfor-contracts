...
import {IPassiveActions} from "./interfaces/IPassiveActions.sol";
    ...
    ERC1155Holder
{
    ...
    function startAction(uint256 playerId, uint16 actionId, uint16 boostItemTokenId)
        external
        override
        isOwnerOfPlayerAndActive(playerId)
    {
        ...
        _activePassiveActions[playerId] = ActivePassiveInfo({
            actionId: actionId, queueId: queueId, startTime: uint40(block.timestamp), boostItemTokenId: boostItemTokenId
        });
    ...
    function claim(uint256 playerId) external override isOwnerOfPlayerAndActive(playerId) nonReentrant {
    ...
    function endEarly(uint256 playerId) external override isOwnerOfPlayerAndActive(playerId) {
        uint256 queueId = _activePassiveActions[playerId].queueId;
        require(queueId != NONE, NoActivePassiveAction());
    ...
    function pendingPassiveActionState(uint256 playerId)
        ...
        view
        override
        returns (PendingPassiveActionState memory _pendingPassiveActionState)
    {
        // If it's not finished then you get nothing
        (bool finished, bool oracleCalled, bool hasRandomRewards, uint256 numWinners, bool skippedToday) =
            finishedInfo(playerId);
        ...
        uint256 guaranteedRewardLength = reward.guaranteedRewardTokenId3 != NONE
            ? 3
            : reward.guaranteedRewardTokenId2 != NONE ? 2 : reward.guaranteedRewardTokenId1 != NONE ? 1 : 0;
        ...
        if (oracleCalled) {
            ...
            bytes memory randomBytes =
                _randomnessBeacon.getRandomBytes(numIterations, passiveAction.startTime, endTime, playerId);
    ...
    function addPassiveActionBridge(uint256 playerId, uint256 actionId, uint256 startTime)
        external
        override
        onlyBridge
    {
    ...
    function _claim(uint256 playerId, uint256 queueId, bool startingAnother) private {
        ...
        for (uint256 i; i < _pendingPassiveActionState.producedRandomRewardItemTokenIds.length; ++i) {
            itemTokenIds[i + _pendingPassiveActionState.producedItemTokenIds.length] =
                _pendingPassiveActionState.producedRandomRewardItemTokenIds[i];
            amounts[i + _pendingPassiveActionState.producedItemTokenIds.length] =
                _pendingPassiveActionState.producedRandomRewardAmounts[i];
        }
        ...
        _activityPoints.rewardBlueTickets(
            ActivityType.passiveactions_evt_claimpassiveaction, msgSender, _players.isPlayerEvolved(playerId), 1
        );
    }
    ...
    function _setupRandomRewards(ActionRewards memory rewards)
        private
        pure
        returns (RandomReward[] memory randomRewards)
    {
        randomRewards = new RandomReward[](4);
        uint256 randomRewardLength;
        if (rewards.randomRewardTokenId1 != 0) {
            randomRewards[randomRewardLength++] =
                RandomReward(rewards.randomRewardTokenId1, rewards.randomRewardChance1, rewards.randomRewardAmount1);
        }
        if (rewards.randomRewardTokenId2 != 0) {
            randomRewards[randomRewardLength++] =
                RandomReward(rewards.randomRewardTokenId2, rewards.randomRewardChance2, rewards.randomRewardAmount2);
        }
        if (rewards.randomRewardTokenId3 != 0) {
            randomRewards[randomRewardLength++] =
                RandomReward(rewards.randomRewardTokenId3, rewards.randomRewardChance3, rewards.randomRewardAmount3);
        }
        if (rewards.randomRewardTokenId4 != 0) {
            randomRewards[randomRewardLength++] =
                RandomReward(rewards.randomRewardTokenId4, rewards.randomRewardChance4, rewards.randomRewardAmount4);
        }
    ...
    function _packAction(PassiveActionInfoInput calldata actionInfo, bool hasRandomRewards)
        private
        pure
        returns (PassiveAction memory passiveAction)
    {
    ...
    function _setAction(PassiveActionInput calldata passiveActionInput) private {
        ...
        _actions[passiveActionInput.actionId] =
            _packAction(passiveActionInput.info, passiveActionInput.randomRewards.length != 0);
    ...
    function addActions(PassiveActionInput[] calldata passiveActionInputs) external override onlyOwner {
    ...
    function editActions(PassiveActionInput[] calldata passiveActionInputs) external override onlyOwner {
