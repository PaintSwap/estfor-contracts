...
interface IQuests is IActivityPointsCaller {
  ...
  function addQuests(
    ...
    IQuests.MinimumRequirement[3][] calldata minimumRequirements
  ) external;
  ...
  function getQuestCompletedRewards(
    uint256 questId
  ) external view returns (uint256[] memory itemTokenIds, uint256[] memory amounts, Skill skillGained, uint32 xpGained);
  function initialize(
