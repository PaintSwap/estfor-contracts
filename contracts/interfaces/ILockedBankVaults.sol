...
interface ILockedBankVaults is ICombatants, IClanMemberLeftCB {
  event AttackVaults(uint256 clanId, uint256 defendingClanId, address from, uint256 leaderPlayerId, uint256 requestId, uint256 pendingAttackId, uint256 attackingCooldownTimestamp, uint256 reattackingCooldownTimestamp, uint256 itemTokenId);
  event SetComparableSkills(Skill[] skills);
  event BattleResult(uint256 requestId, uint64[] attackingPlayerIds, uint64[] defendingPlayerIds, uint256[] attackingRolls, uint256[] defendingRolls, uint8[] battleResults, uint8[] randomSkills, bool didAttackersWin, uint256 attackingClanId, uint256 defendingClanId, uint256[] randomWords, uint256 percentageToTake, uint256 brushLost, int256 attackingMMRDiff, int256 defendingMMRDiff, uint256 clanXPGainedWinner);
  ...
  function initialize(IPlayers players, IClans clans, IBrushToken brush, address bankRelay, ItemNFT itemNFT, address treasury, address dev, address paintswapVRFConsumer, Skill[] calldata comparableSkills, uint16 mmrAttackDistance, uint24 lockFundsPeriod, uint8 maxClanCombatants, uint8 maxLockedVaults, AdminAccess adminAccess, IActivityPoints activityPoints, bool isBeta) external;
  ...
  function attackVaults(uint256 clanId, uint256 defendingClanId, uint16 itemTokenId, uint256 leaderPlayerId) external payable;
