...
import {IGameSubsidisationRegistry} from "./dependencies/interfaces/IGameSubsidisationRegistry.sol";

/// @title UsageBasedSessionModule
/// @notice A module for Gnosis Safe that allows for session keys with rate-limited actions
contract UsageBasedSessionModuleV1 is
  UUPSUpgradeable,
  OwnableUpgradeable,
  EIP712Upgradeable,
  ...
  PausableUpgradeable
{
  ...
  ) external nonReentrant whenNotPaused returns (uint256 gasUsed, uint256 successCount, bytes[] memory errors) {
    require(_whitelistedSigners[msg.sender], UnauthorizedSigner());
    require(params.length > 0, NoBatchItems());
    require(params.length <= MAX_BATCH_SIZE, BatchTooLarge());
