...
contract TestPaintSwapDecorator is Ownable, IPaintSwapDecorator {
  using SafeMath for uint256;
  ...
  function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public onlyOwner {
    ...
    poolInfo_.push(
      PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardTime: lastRewardTime, accBrushPerShare: 0})
    );
  ...
  function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate) public onlyOwner {
    require(poolExists(address(poolInfo_[_pid].lpToken)), "pid does not yet exist");
    if (_withUpdate) {
      massUpdatePools();
    }
    totalAllocPoint = totalAllocPoint.sub(poolInfo_[_pid].allocPoint).add(_allocPoint);
    poolInfo_[_pid].allocPoint = _allocPoint;
  }
  ...
  function massUpdatePools() public {
    uint256 length = poolInfo_.length;
  ...
  function withdraw(uint256 _pid, uint256 _amount) public {
    ...
    uint256 withdrawFee = _withdraw(_amount, pool.lpToken);
    user.rewardDebt = user.amount.mul(pool.accBrushPerShare).div(1e12);
  ...
  function poolInfo(
    ...
    view
    override
    returns (address lpToken, uint256 allocPoint, uint256 lastRewardTime, uint256 accBrushPerShare)
  {
    return (
      address(poolInfo_[_pid].lpToken),
      poolInfo_[_pid].allocPoint,
      poolInfo_[_pid].lastRewardTime,
      poolInfo_[_pid].accBrushPerShare
    );
  }
}
