...
import {ILockedBankVaultsBattleResultDecoder, LockedBankVaultsBattleResultData, RaidBattleOutcomeData, TerritoryBattleResultData} from "./interfaces/LockedBankVaultsBattleResultDecoder.sol";
...
contract LockedBankVaultsBattleResultDecoder is ILockedBankVaultsBattleResultDecoder {
  ...
  function decodeRaidOutcome(bytes calldata data) external pure returns (RaidBattleOutcomeData memory result) {
    ...
    ) = abi.decode(data, (uint256, uint256, uint256, uint256, uint256, uint16[], uint256, bool, uint256[], uint256[]));
  }

  function decodeTerritoryBattleResult(
    bytes calldata data
  ) external pure returns (TerritoryBattleResultData memory result) {
    (
      result.requestId,
      result.attackingPlayerIds,
      result.defendingPlayerIds,
