...
import {IInstantActions} from "./interfaces/IInstantActions.sol";
...
contract InstantActions is IInstantActions, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize(IPlayers players, ItemNFT itemNFT, IQuests quests, IActivityPoints activityPoints)
        external
        override
        initializer
    {
    ...
    function getAction(InstantActionType actionType, uint16 actionId)
        external
        view
        override
        returns (InstantAction memory)
    {
    ...
    function doInstantActions(
        ...
        _activityPoints.rewardBlueTickets(
            ActivityType.instantactions_evt_doinstantactions, msgSender, _players.isPlayerEvolved(playerId), 1
        );
    }
    ...
    function getInstantActionState(
        ...
        uint256[] calldata amounts,
        InstantActionType actionType
    ) public view override returns (InstantActionState memory instantActionState) {
    ...
    function _genericInstantActionState(uint256 playerId, uint16[] calldata actionIds, uint256[] calldata amounts)
        private
        view
        returns (InstantActionState memory instantActionState)
    {
    ...
    function _forgingCombineActionState(uint256 playerId, uint16[] calldata actionIds, uint256[] calldata amounts)
        private
        view
        returns (InstantActionState memory instantActionState)
    {
    ...
    function _packAction(InstantActionInput calldata actionInput)
        private
        pure
        returns (InstantAction memory instantAction)
    {
    ...
    function _checkInputs(InstantActionInput calldata actionInput) private pure {
        (uint16[] calldata inputTokenIds, uint24[] calldata amounts) =
            (actionInput.inputTokenIds, actionInput.inputAmounts);
    ...
    function addActions(InstantActionInput[] calldata instantActionInputs) external override onlyOwner {
    ...
    function editActions(InstantActionInput[] calldata instantActionInputs) external override onlyOwner {
    ...
    function removeActions(InstantActionType[] calldata actionTypes, uint16[] calldata instantActionIds)
        external
        override
        onlyOwner
    {
