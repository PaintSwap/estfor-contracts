...
import {IInstantVRFActions} from "./interfaces/IInstantVRFActions.sol";
...
contract InstantVRFActions is IInstantVRFActions, UUPSUpgradeable, OwnableUpgradeable, PaintswapVRFConsumerUpgradeable {
        ...
        isOwnerOfPlayerAndActive(playerId)
    {
        ...
        emit DoInstantVRFActions(
            msgSender, playerId, uint256(requestId), actionIds, actionAmounts, consumedItemTokenIds, consumedAmounts
        );

        // issue activity points
        _activityPoints.rewardBlueTickets(
            ActivityType.instantvrfactions_evt_doinstantvrfactions, msgSender, isPlayerEvolved, 1
        );
    }
    ...
    function _fulfillRandomWords(uint256 requestId, uint256[] calldata randomWords) internal override {
        ...
        (uint256[] memory itemTokenIds, uint256[] memory itemAmounts, uint256[] memory petBaseIds) =
        ...
        if (itemTokenIds.length != 0) {
            try _itemNFT.mintBatch(from, itemTokenIds, itemAmounts) {}
            catch {
    ...
    function _getRewards(uint256 playerId, uint256[] calldata randomWords)
        private
        view
        returns (uint256[] memory itemTokenIds, uint256[] memory itemAmounts, uint256[] memory petBaseIds)
    {
        ...
        for (uint256 i; i < playerActionInfoLength / 2; ++i) {
            ...
            ) = IInstantVRFActionStrategy(strategy)
                .getRandomRewards(actionId, actionAmount, randomWords, randomWordStartIndex);
    ...
    function _requestRandomWords(uint256 numRandomWords, uint256 numActions) private returns (uint256 requestId) {
        requestId = _requestRandomnessPayInNative(
            callbackGasLimitForRequests(numActions), numRandomWords, DAO_MULTISIG_ADDRESS, msg.value
        );
    }
    ...
    function _packAction(InstantVRFActionInput calldata actionInput)
        private
        pure
        returns (InstantVRFAction memory instantVRFAction)
    {
    ...
    function requestCost(uint256 numActions) public view override returns (uint256) {
        ...
        onlyOwner
    {
        ...
        for (uint256 i; i < instantVRFActionTypes.length; ++i) {
            require(
                instantVRFActionTypes[i] != InstantVRFActionType.NONE && strategies[i] != address(0), InvalidStrategy()
            );
    ...
    function addActions(InstantVRFActionInput[] calldata instantVRFActionInputs) external override onlyOwner {
    ...
    function editActions(InstantVRFActionInput[] calldata instantVRFActionInputs) external override onlyOwner {
    ...
    function removeActions(uint16[] calldata instantVRFActionIds) external override onlyOwner {
    ...
    function setGasCostPerUnit(uint64 gasCostPerUnit) public override onlyOwner {
        _gasCostPerUnit = uint64(gasCostPerUnit);
        emit SetGasCostPerUnit(gasCostPerUnit);
    }

    function setPetNFT(address petNFT) external override onlyOwner {
        _petNFT = PetNFT(petNFT);
    }

    function setMaxActionAmount(uint8 maxActionAmount) public override onlyOwner {
