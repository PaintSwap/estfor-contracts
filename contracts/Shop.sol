...
contract Shop is UUPSUpgradeable, OwnableUpgradeable, IActivityPointsCaller {
  ...
  function getShopItemStates(uint256 startTokenId, uint256 endTokenId) external view returns (ShopItemState[] memory) {
    ...
    for (uint256 tokenId = startTokenId; tokenId < endTokenId; ++tokenId) {
      uint256 price = _shopItems[uint16(tokenId)];
      TokenInfo memory tokenInfo = _tokenInfos[uint16(tokenId)];
  ...
  function buySupporterPack(uint24 packId, address to, uint16 amount) external {
    ...
    require(block.timestamp >= pack.startTimestamp, SupporterPackNotStarted(pack.startTimestamp, block.timestamp));
  ...
  function setSupporterPacks(uint24[] calldata packIds, SupporterPack[] calldata packs) external onlyOwner {
