...
contract CombatMeleeTest is CombatTestBase {
    ...
    function testInProgressCombatUpdatesMany() public {
        ...
        assertEq(
            itemNFT.balanceOf(ALICE, BRONZE_ARROW), (action.timespan * rate * spawned) / (3600 * GUAR_MUL * SPAWN_MUL)
        );
        assertEq(itemNFT.balanceOf(ALICE, COOKED_MINNUS), 254);
    }
    ...
    function testClearEverything() public {
        ...
        players.startActionsAdvanced(
            playerId, _actions(natuow), XP_BOOST, BOOST_START_NOW, 0, raffleCost, ActionQueueStrategy.OVERWRITE
        );
        vm.stopPrank();
    ...
    function _setupNatuowAndGrog() private returns (QueuedActionInput memory natuow, QueuedActionInput memory grog) {
        ...
        combatActions[1] =
            _productionCombatAction(ACTION_COMBAT_GROG_TOAD, 120, CombatStats(3, 0, 0, 50, 3, 0, 0), false);
    ...
    function _productionCombatAction(uint16 actionId, uint24 xpPerHour, CombatStats memory stats, bool isNatuow)
        private
        pure
        returns (ActionInput memory action)
    {
    ...
    function _boostItem(uint16 tokenId, EquipPosition position, BoostType boostType, uint16 value)
        private
        pure
    ...
    function _testFoodCap(bool healing, bool split) private {
        ...
        _addCombatAction(
            2, uint24(100 * SPAWN_MUL), CombatStats(15000, 0, 0, 5, 0, 0, 0), BRONZE_ARROW, uint16(GUAR_MUL), false
        );
        action.actionId = 2;
