...
interface PlayersImplProcessActions {
    function donate(address from_, uint256 playerId, uint256 amount) external;
    function processActions(address from_, uint256 playerId)
        external
        returns (QueuedAction[] memory remainingQueuedActions, PendingQueuedActionData memory currentActionProcessed);
    function processActionsAndSetState(address from_, uint256 playerId)
        external
        returns (QueuedAction[] memory remainingQueuedActions, Attire[] memory remainingAttire);
