...
interface PlayersImplRewards {
    function claimRandomRewards(
        address from_,
        uint256 playerId,
        PendingQueuedActionProcessed calldata pendingQueuedActionProcessed
    ) external;
    function pendingQueuedActionStateImpl(address owner, uint256 playerId)
        external
        view
        returns (PendingQueuedActionState memory pendingQueuedActionState);
