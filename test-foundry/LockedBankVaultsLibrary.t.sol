...
contract LockedBankVaultsLibraryTest is Test {
  ...
  function testGetNewMMRsExtremes() public pure {
  ...
  function testGetNewMMRsVariousChecks() public pure {
  ...
  function testGetNewMMRsSmallKValues() public pure {
    _assertMMRs(3, 3, 500, 498, true, 501, 497);
    _assertMMRs(3, 3, 500, 498, false, 499, 499);
  }

  function testIsWithinRangeReturnsTrueWhenDefenderIsWithinRange() public pure {
    assertTrue(_isWithin(_standardMMRs(), 2, 4, 2));
  }

  function testIsWithinRangeReturnsFalseWhenDefenderIsOutOfRange() public pure {
    assertFalse(_isWithin(_standardMMRs(), 2, 5, 2));
  }

  function testIsWithinRangeHandlesBeginningOfArray() public pure {
    assertFalse(_isWithin(_standardMMRs(), 0, 2, 1));
  }

  function testIsWithinRangeHandlesEndOfArray() public pure {
    assertFalse(_isWithin(_standardMMRs(), 5, 3, 1));
  }

  function testIsWithinRangeIncludesSameMMRAtEdge() public pure {
    assertTrue(_isWithin(_mmrs(1_000, 1_100, 1_200, 1_300, 1_300, 1_400, 1_500), 2, 4, 1));
  }

  function testIsWithinRangeIncludesMultipleSameMMRsAtEdge() public pure {
  ...
  function testIsWithinRangeIncludesSameMMRAtEdgeInReverse() public pure {
    assertTrue(_isWithin(_mmrs(1_000, 1_100, 1_200, 1_200, 1_300, 1_400, 1_500), 4, 2, 1));
  }

  function testIsWithinRangeExcludesDifferentMMROutsideRange() public pure {
    assertFalse(_isWithin(_standardMMRs(), 2, 4, 1));
  }

  function testIsWithinRangeIncludesSameMMRAtBeginningOfRange() public pure {
  ...
  function testIsWithinRangeIncludesSameMMRAtBeginningOfRangeExtremes() public pure {
