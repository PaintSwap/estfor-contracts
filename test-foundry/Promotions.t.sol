...
contract PromotionTest is FullGameStack {
    ...
    function testSinglePromotionMintsOneRandomItem() public {
        ...
        uint256 total = itemNFT.balanceOf(ALICE, HALLOWEEN_BONUS_1) + itemNFT.balanceOf(ALICE, HALLOWEEN_BONUS_2)
    ...
    function testStreakRewardDoesNotChangeDuringClaimPeriod() public {
        ...
        (uint256[] memory itemTokenIds,, uint256[] memory daysToSet, PromotionMintStatus status) =
            promotions.mintPromotionViewNow(playerId, Promotion.XMAS_2023);
    ...
    function _promotions(PromotionInfoInput memory promotion)
        private
        pure
        returns (PromotionInfoInput[] memory promotions_)
    {
    ...
    function _equipment(uint16 itemTokenIdA, uint24 amountA, uint16 itemTokenIdB, uint24 amountB)
        private
        pure
        returns (Equipment[] memory values)
    {
