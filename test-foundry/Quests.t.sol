...
contract QuestsTest is FullGameStack {
    ...
    function testCookedFoodGivingAwayQuestCheckCombatConsumingTheCookedFoodBeforeQuestCompleted() public {
        ...
        (QueuedActionInput memory queuedActionMelee,) =
            _addCombatAction(2, 100 * SPAWN_MUL, CombatStats(100, 0, 0, 1000, 0, 0, 0), 2, COOKED_MINNUS);

        vm.startPrank(ALICE);
        players.startActions(
            playerId, _queuedActions(queuedActionCooking, queuedActionMelee), ActionQueueStrategy.OVERWRITE
        );
        vm.stopPrank();
    ...
    function _setupBasicWoodcutting(uint256 rate)
        private
        returns (QueuedActionInput memory queuedAction, uint256, uint16)
    {
        ActionInput[] memory actions = new ActionInput[](1);
    ...
    function _setupBasicFletching(uint256 rate, uint256 outputAmount)
        private
        returns (QueuedActionInput memory queuedAction, uint256, uint16 choiceId)
    {
        ActionInput[] memory actions = new ActionInput[](1);
        actions[0].actionId = 1;
