...
contract LockedBankVaultsLibraryTest is EstforTest {
    ...
    function _assertMMRs(
        ...
        (uint16 attackerMMR, uint16 defenderMMR) =
            lockedBankVaultsLibrary.getNewMMRs(kA, kD, attackingMMR, defendingMMR, attackersWon);
        assertEq(attackerMMR, expectedAttackerMMR);
        assertEq(defenderMMR, expectedDefenderMMR);
    }

    function _isWithin(uint16[] memory mmrs, uint256 clanId, uint256 defendingClanId, uint256 distance)
        private
        view
        returns (bool)
    {
    ...
    function _mmrs(uint16 a, uint16 b, uint16 c, uint16 d, uint16 e, uint16 f)
        private
        pure
    ...
    function _mmrs(uint16 a, uint16 b, uint16 c, uint16 d, uint16 e, uint16 f, uint16 g)
        private
        pure
        returns (uint16[] memory values)
    {
    ...
    function _mmrs(uint16 a, uint16 b, uint16 c, uint16 d, uint16 e, uint16 f, uint16 g, uint16 h, uint16 i)
        private
        pure
        returns (uint16[] memory values)
    {
    ...
    function _mmrs(uint16 a, uint16 b, uint16 c, uint16 d, uint16 e, uint16 f, uint16 g, uint16 h, uint16 i, uint16 j)
        private
        pure
        returns (uint16[] memory values)
    {
