...
contract BlackMarketTraderTest is FullGameStack {
    ...
    function _item(uint16 id, uint16 amount, uint128 price, uint16 stock, bool active)
        private
        pure
        returns (BlackMarketTrader.ShopItem memory)
    {
        return BlackMarketTrader.ShopItem(price, id, amount, stock, stock, active);
    }

    function _shopItems(BlackMarketTrader.ShopItem memory item)
        private
        pure
        returns (BlackMarketTrader.ShopItem[] memory items)
    {
