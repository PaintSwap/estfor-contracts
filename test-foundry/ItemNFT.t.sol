...
contract ItemNFTTest is EstforTest {
    ...
    function testIsApprovedForAllOverride() public {
        ...
        vm.prank(ALICE);
        vm.expectRevert(
            abi.encodeWithSelector(IERC1155Errors.ERC1155MissingApprovalForAll.selector, ALICE, address(this))
        );
    ...
    function testTransferOfItemsToManyDifferentUsersAtOnce() public {
        ...
        infos[0] = BulkTransferInfo({
            tokenIds: _uints(TITANIUM_AXE, RUNITE_AXE, ORICHALCUM_AXE), amounts: _uints(2, 1, 2), to: ALICE
        });
